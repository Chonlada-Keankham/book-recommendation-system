...
import { BadRequestException, ConflictException, Inject, Injectable, NotFoundException, Put, Res, UploadedFile, UseInterceptors, forwardRef } from '@nestjs/common';
...
export class BookService {
  ...
  async createBook(createBookDto: CreateBookDto, filename: string): Promise<iBook> {
    ...
    const BACKEND_URL = process.env.BACKEND_URL || 'http://localhost:5000';  // ใช้ URL ของ CDN
    const newBook = new this.bookModel({
      ...createBookDto,
      img: `${BACKEND_URL}/uploads/book${filename}`, // เปลี่ยนไปใช้ URL ของ CDN
      deleted_at: null
    });
  ...
  async findOneByIdAndUpdateView(id: string, ip: string): Promise<iBook> {
    ...
    const redisKey = `book:${id}`;
    const cachedBook = await this.redisClient.get(redisKey);
    if (cachedBook) {
      return JSON.parse(cachedBook);  // ถ้ามีข้อมูลใน Redis ให้ใช้ข้อมูลนั้น
    }
    ...
    // แคชข้อมูลใน Redis
    await this.redisClient.set(redisKey, JSON.stringify(book), 'EX', 3600);  // เก็บไว้ในแคช 1 ชั่วโมง
  ...
  async findBooksByCategory(category: BookCategory, ip: string): Promise<{
    ...
    // เพิ่มการจัดการกับ Redis เพื่อไม่ให้ยอดวิวซ้ำ
    for (const book of books) {
      const redisKey = `viewed:${book._id}:${ip}`;
      const viewed = await this.redisClient.get(redisKey);
      ...
      // ตรวจสอบว่า img มีหรือไม่ และปรับ URL
      if (book.img) {
        book.img = `${process.env.BACKEND_URL}/uploads/book${book.img}`;
      }
    }
  ...
  async uploadBookCover(bookId: string, filename: string): Promise<iBook> {
    ...
    const compressedImageBuffer = await sharp(book.img)  // บีบอัดไฟล์ภาพ
      .resize(800)  // กำหนดขนาดภาพใหม่
      .toBuffer();

    // อัปโหลดภาพที่บีบอัดไปที่เซิร์ฟเวอร์หรือ CDN
    await sharp(compressedImageBuffer).toFile('path_to_compressed_image.jpg');  // ตัวอย่างการบีบอัดภาพ
